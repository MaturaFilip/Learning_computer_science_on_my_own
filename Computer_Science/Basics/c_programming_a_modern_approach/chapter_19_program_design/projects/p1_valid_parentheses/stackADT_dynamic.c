...
PUBLIC void push(Stack s, Item i) {
    if (is_full(s)) {
        ...
        printf("Array is full. Capacity: %d, doubling the size to: %d\n", s->size, s->size * 2);
        int new_capacity = s->size * 2;
        Item *n = realloc(s->contents, sizeof(Item) * new_capacity);
        if (n == NULL) {
            terminate("Fail during array extension.");
        }
        s->contents = n;
        s->size = new_capacity;
    }
