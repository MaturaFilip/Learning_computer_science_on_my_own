...
void print(struct part inventory[], int num_parts) {
    int i;
    int parts_ids[num_parts];
    for (i = 0; i < num_parts; i++) {
        parts_ids[i] = inventory[i].number;
    }
    int *s = parts_ids;
    int *e = parts_ids + (num_parts - 1);

    quicksort(parts_ids, s, e);

    printf("Part Number     Part Name                   Quantity on Hand        Price\n");

    for (i = 0; i < num_parts; i++) {
        int j = 0;
        while (inventory[j].number != parts_ids[i]) {
            j++;
        }
        printf("%7d             %-25s%11d%17.2lf\n", inventory[j].number, inventory[j].name, inventory[j].on_hand, inventory[j].price);
    }
}
